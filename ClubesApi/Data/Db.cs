    ...
    public class Db
    {
        ...
        public Club? CreateClub(ClubCreateDto dto)
        {
            var sql = @"
                ...
                VALUES (@Nombre, @CantidadSocios, @CantidadTitulos, @FechaFundacion, @UbicacionEstadio, @NombreEstadio, 1);
            ...
            using (var command = new SqlCommand(sql, connection))
            {
                ...
                command.Parameters.AddWithValue("@CantidadSocios", dto.CantidadSocios);
                command.Parameters.AddWithValue("@CantidadTitulos", dto.CantidadTitulos);
        ...
        public bool UpdateClub(Club club)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var command = new SqlCommand(
                    "UPDATE Clubes SET Nombre = @Nombre, CantidadSocios = @CantidadSocios, CantidadTitulos = @CantidadTitulos, FechaFundacion = @FechaFundacion, UbicacionEstadio = @UbicacionEstadio, NombreEstadio = @NombreEstadio, IsActive = @IsActive WHERE ClubId = @ClubId",
                    connection);
